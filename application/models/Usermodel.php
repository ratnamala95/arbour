...
class Usermodel extends CI_Model {
    ...
    public function registrationdata()
 	{
 		$distId = trim($this->input->post('distributor_id'));

 		//fint warehouse id
 		$rowUserCount = $this->db->query('select warehouse_id AS WAREHOUSEID from users where role = '.DISTRIBUTOR_ROLE.' and  id= '.$distId);
 		//echo $this->db->last_query();
 		$results = $rowUserCount->row();
 		$resultwarehouseid = $results->WAREHOUSEID;
 	
 		// find user count
 		
 		$rowUserCount = $this->db->query('select count(id) AS TOTALRECORDCOUNT from users where role = '.RETAILER_ROLE.' and  distributor_id= '.$distId);
 		$results = $rowUserCount->row();
 		$totalUsersCount = $results->TOTALRECORDCOUNT;
 		$totalUsersCount++;
		...
		$userdata['emailexists'] = $query->result();
 		if (empty($userdata['emailexists'])) {
 			...
 			$data['role'] = $this->input->post('role');
  			...
  			$data['user_count'] = $totalUsersCount;
  			$data['warehouse_id']=$resultwarehouseid;
