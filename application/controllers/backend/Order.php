...
class Order extends Backend_Controller {
    ...
    $count['counter'] = array();
		...
		{
			$aCond['orders.distributor_id'] = $aUsr['id'];
			$aRows = $this->Dbaction->getAllData('orders','',array('distributor_id' => $aUsr['id']));
			$aRetailer = $this->Dbaction->getAllData("users",'',array("role" => RETAILER_ROLE,"status" => 1,'distributor_id' => $aUsr['id']));
		}
		elseif($aUsr['role']==WAREHOUSE_ROLE)
		{
			$aCond['wrehouse_id'] = $aUsr['id'];
			$aRows = $this->Dbaction->getAllData('orders','',array('wrehouse_id' => $aUsr['id']));
			$aRetailer = $this->Dbaction->getAllData("users",'',array("role" => RETAILER_ROLE,"status" => 1,'warehouse_id' => $aUsr['id']));
		}
		else {
	...
	/*	$aRetailer = $this->Dbaction->getAllData("users",'',array("role" => RETAILER_ROLE,"status" => 1)); */
		...
		$this->title   = 'Orders';
		$this->data    = array("aRows" => $aRows,"aWare" => $aWare,'aDistributor'=>$aDistributor,'aRetailer'=>$aRetailer,'aVals'=>$aVals,"aUsr"=>$aUsr);
		$this->layout();
	}
		...
		if(isset($_FILES['userfile']) && $_FILES['userfile'])
	 	{
		    ...
		    redirect(getSiteUrl('order/upload_file/'.$orderId), 'refresh');
		}

		$aRowData = $this->Dbaction->getAllData('order_document','',['doc_order_id'=>$orderId],'','',$aOrders=array('doc_id'=>'DESC'));
		//print_r($aRowData);
		$this->content = 'orders/upload_file';
		$this->title = 'Upload Files';
		$this->data = array('aRowData'=>$aRowData);
		$this->layout();
	}
