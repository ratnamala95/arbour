...
class Product extends Frontend_Controller
{
	...
	public function index()
	{
		...
		if ($this->uri->segment(2)) {
			...
			$data['productResults'] = $this->Dbaction->getSingleData("products","products.*,category.name as category_name,inventory.price AS price, inventory.sale_price AS sale_price,inventory.size AS size,inventory.id AS inventory_id,pieces_per_set,gst",$acondition,$aJoins);
