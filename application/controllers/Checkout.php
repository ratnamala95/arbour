...
class Checkout extends Frontend_Controller
{
	...
	/*	$aCons = array(
		...
		$retailerId = $this->session->userdata('REATILER_LOGIN_ID');
	    $warehouseId = $this->session->userdata('WAREHOUSE_ID');
	    
		$rowData = $this->db->query('SELECT * FROM cart LEFT JOIN products ON products.id=cart.cart_pro_id LEFT JOIN inventory ON inventory.product_name=cart.cart_pro_id where cart_retailer_id='.$retailerId.' AND cart_warehouse_id='.$warehouseId.' GROUP BY cart_pro_id ORDER BY cart.cart_id DESC');
	...
	public function transaction()
	{
		...
		if ($this->form_validation->run()) {
			...
			$wareHouseCount= $this->Dbaction->getSingleData('users','user_count',['id'=>$this->session->userdata('WAREHOUSE_ID')]);
			$strWarehouseCountId = ($wareHouseCount['user_count']/10);
			if (number_format((int)$strWarehouseCountId)==0) {
				$firstNumber='0'.$wareHouseCount['user_count'];
			} else {
				$firstNumber=$wareHouseCount['user_count'];
			}

			$distributoreCount= $this->Dbaction->getSingleData('users','user_count',['id'=>$this->session->userdata('DISTRIBUTOR_ID')]);
			$strDistributoreCountId = ($distributoreCount['user_count']/10);
			if (number_format((int)$strDistributoreCountId)==0) {
				$secondNumber='0'.$distributoreCount['user_count'];
			} else {
				$secondNumber=$distributoreCount['user_count'];
			}

			$retailerCount= $this->Dbaction->getSingleData('users','user_count',['id'=>$this->session->userdata('REATILER_LOGIN_ID')]);
			$retailerCount['user_count'];
			$strRetailerCountId = ($retailerCount['user_count']/10);
			if (number_format((int)$strRetailerCountId)==0) {
				$thirdNumber='0'.$retailerCount['user_count'];
			} else {
				$thirdNumber=$retailerCount['user_count'];
			}
			...
			$orderdata['trans_date']=$now;
			$orderdata['orders_status']=0;

			$lastInsertId = $this->Dbaction->adddata('orders',$orderdata);
            
            $customerTransOrderId= 'ORD # '.$firstNumber.'-'.$secondNumber.'-'.$thirdNumber.'-'.$lastInsertId;
            $aVals = array(
                        'customer_trans_order_id'=>$customerTransOrderId,
                    );
            $this->Dbaction->updateData('orders',$aVals,$aCons = array('orders_id'=>$lastInsertId));
			...
			foreach ($this->input->post('id') as $key=>$items) {
				...
				$detailsData['od_inventory_id']=$this->input->post('inventory_id')[$key];
                ...
                $detailsData['od_product_order_date']=$now;
